...
describe("platform session", () => {
  ...
  it("builds continue URLs for Connect and Suite", () => {
    ...
    process.env.REGIMA_LMS_URL = "http://localhost:5000";
    ...
    expect(urls.hub).toBe("http://localhost:5000/platform");
    expect(urls.chain).toContain("/api/platform/login?session=");
    const suiteNext = new URL(urls.chain as string).searchParams.get("next");
    expect(suiteNext).toContain("/login?platform_session=");
    expect(new URL(suiteNext as string).searchParams.get("next")).toBe(
